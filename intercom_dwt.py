...
class Intercom_DWT(Intercom_empty):
    ...
    def init(self, args):
        ...
        self.coeffs = pywt.wavedec(np.zeros(self.frames_per_chunk), "db1", mode="per", level=4)
        ...
        for cell in range(self.cells_in_buffer):
            self.buffer_coeffs[cell] = np.zeros((self.frames_per_chunk, 2), dtype=np.int32)

    def receive_and_buffer(self):
        # Same as intercom_dfc
        message, source_address = self.receiving_sock.recvfrom(Intercom_empty.MAX_MESSAGE_SIZE)
        received_chunk_number, received_bitplane_number, self.NORB, *bitplane = struct.unpack(self.packet_format, message)
        bitplane = np.asarray(bitplane, dtype=np.uint8)
        bitplane = np.unpackbits(bitplane)

        # We have to change from int16 to int32 type
